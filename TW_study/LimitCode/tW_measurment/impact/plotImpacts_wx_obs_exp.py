...
for page in xrange(n):
    ...
    legend.AddEntry(g_pulls, 'Pull obs.', 'LP')
    legend.AddEntry(g_impacts_hi, '+1#sigma Impact obs.', 'F')
    legend.AddEntry(g_impacts_lo, '-1#sigma Impact obs.', 'F')
    legend.AddEntry(g_pulls_e, 'Pull exp.', 'LP')
    legend.AddEntry(g_impacts_hi_e, '+1#sigma Impact exp.', 'F')
    legend.AddEntry(g_impacts_lo_e, '-1#sigma Impact exp.', 'F')
    legend.Draw()
...
#    plot.DrawTitle(pads[1], '#hat{%s_{exp.}} = %.3g #pm %.3g%s' % (Translate(POIs[0], translate), POI_fit[1], (POI_fit[2] - POI_fit[0]) / 2., '' if args.units is None else ' '+args.units), 3)
    plot.DrawTitle_v1(pads[1], '#hat{%s}_{obs.} = %.3g #pm %.3g%s, #hat{%s}_{exp.} = %.3g #pm %.3g%s' % (Translate(POIs[0], translate), POI_fit[1], (POI_fit[2] - POI_fit[0]) / 2., '' if args.units is None else ' '+args.units, Translate(POIs[0], translate), POI_fit_e[1], (POI_fit_e[2] - POI_fit_e[0]) / 2., '' if args.units is None else ' '+args.units), 3)
    extra = ''
    if page == 0:
        extra = '('
    if page == n - 1:
