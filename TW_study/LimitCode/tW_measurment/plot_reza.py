...
file=ROOT.TFile("./data_card_comb/postfit_shapes_emu.root","r")
#file=ROOT.TFile("./data_card_comb/postfit_shapes_emu_11_21_22.root","r")

adapt=ROOT.gROOT.GetColor(12)
new_idx=ROOT.gROOT.GetListOfColors().GetSize() + 1
trans=ROOT.TColor(new_idx, adapt.GetRed(), adapt.GetGreen(),adapt.GetBlue(), "",0.5)

#categories=["emu_1jet_1bjet_prefit","emu_1jet_1bjet_postfit"] 
categories=["Name1_prefit","Name1_postfit","Name2_prefit","Name2_postfit","Name3_prefit","Name3_postfit","Name4_prefit","Name4_postfit"] 
#categories=["Name1_prefit","Name1_postfit","Name2_prefit","Name2_postfit","Name3_prefit","Name3_postfit"] 
ncat=len(categories)


for i in range (0,ncat):
   Data=file.Get(categories[i]).Get("data_obs")
   other=file.Get(categories[i]).Get("other")
   ...
   errorBand=file.Get(categories[i]).Get("jets").Clone()
   errorBand.Add(other)
   errorBand.Add(DY)
   errorBand.Add(TT)
   errorBand.Add(TW)
...
#   legende.AddEntry(Data,"Observed","elp")
   ...
   h1.Divide(hwoE)
   h3.Divide(hwoE)
   h1.GetXaxis().SetTitle("NN output")#m_{vis}(#tau_{h},#tau_{h}) (GeV)")#(#vec{p_{T}}(#tau_{1})+#vec{p_{T}}(#tau_{2}))/(p_{T}(#tau_{1})+p_{T}(#tau_{2}))")#("m_{vis} (GeV)")#(#vec{p_{T}(#mu)}+#vec{p_{T}(#tau)})/(p_{T}(#mu)+p_{T}(#tau))")
...
#   c.SaveAs("./plot_pre_post_fit/"+categories[i]+".pdf")
   c.SaveAs("./plot_pre_post_fit/"+categories[i]+".png")
