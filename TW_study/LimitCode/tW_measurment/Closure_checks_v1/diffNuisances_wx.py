...
parser.add_option("-n", "--name", dest="name", default=None, type="string", help="If true, create sorted rho output file.")##########wx
...
for i in range(fpf_s.getSize()):
    ...
    row += [ "%+4.2f"  % fit_s.correlation(name, options.poi.strip(" ")) ]
...
########## Wx sorted rho output #####
if options.name != None:
    name=options.name.strip(" ")
    F_out=open("%s.txt"%name,"w")
    F_out.write(fmtstring % ('name', 'b-only fit', 's+b fit', 'rho'))
    tmp=sorted(table.items(),key=lambda item:abs(float(item[1][2])),reverse=True)
    for i in tmp:
        n=i[0]
        v=i[1]
        F_out.write("\n"+fmtstring % (n, v[0], v[1], v[2]))
    F_out.close() 
