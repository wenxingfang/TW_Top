...
#    line_2sigma.Draw()
    ...
    if Unblind:
        ...
        print "obs up:%-5s_%-8s%-15s| best: %.2f | 1sigma:%.2f to %.2f | 2sigma:%.2f to %.2f|"%(coulp,chan,region,float(best_obs_up),float(Sigma1_x_min_obs_up),float(Sigma1_x_max_obs_up),float(Sigma2_x_min_obs_up),float(Sigma2_x_max_obs_up))
        print "obs down:%-5s_%-8s%-15s| best: %.2f | 1sigma:%.2f to %.2f | 2sigma:%.2f to %.2f|"%(coulp,chan,region,float(best_obs_down),float(Sigma1_x_min_obs_down),float(Sigma1_x_max_obs_down),float(Sigma2_x_min_obs_down),float(Sigma2_x_max_obs_down))
...
#Dir="/user/wenxing/Limits/CMSSW_7_4_7/src/LimitCode/TOP_EFT_0629/scan_NoTopPtReW/"##for no top pt reweight check
for_paper_style=True
Unblind=True
Add_up_down=False ## true for nominal
