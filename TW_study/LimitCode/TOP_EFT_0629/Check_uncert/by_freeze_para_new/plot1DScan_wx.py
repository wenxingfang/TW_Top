...
parser.add_argument('--y-cut', type=float, default=4.5, help='Remove points with y > y-cut')
parser.add_argument('--y-max', type=float, default=2., help='y-axis maximum')
...
parser.add_argument('--Xtitle', default='r')
...
axishist.GetXaxis().SetTitle("%s" % args.Xtitle)
...
if len(other_scans) > 0:
    ...
    axishist.GetXaxis().SetLimits(new_min, 6*new_max)
...
#box = ROOT.TBox(axishist.GetXaxis().GetXmin(), 0.9*args.y_max, axishist.GetXaxis().GetXmax(), args.y_max)
#box.Draw()
...
textfit = '%s = %.3f(#plus %.3f)(#minus %.3f)' % (fixed_name, val_nom[0], val_nom[1], abs(val_nom[2]))

value_dict={}
value_dict['main']=textfit
...
if args.breakdown is not None:
    ...
    for i, br in enumerate(breakdown):
        if i < (len(breakdown) - 1):
            ...
            hi = v_hi[i]
            lo = v_lo[i]
        textfit += '{}^{#plus %.3f}_{#minus %.3f}(%s)\n' % (hi, abs(lo), br)
        textfit_1 = '(#plus %.3f)(#minus %.3f)(%s)' % (hi, abs(lo), br)
        value_dict[br]=textfit_1
        pt.AddText(textfit_1)
    #pt.AddText(textfit)
    print textfit
...
cms_text_sixe=0.05
label_x_l=0.11
Label_cms = ROOT.TLatex(label_x_l,0.95,label_cms)
Label_cms.SetNDC()
Label_cms.SetTextSize(cms_text_sixe)
Label_cms.SetTextFont(61)
Label_cms.Draw()
Label_prelim = ROOT.TLatex(label_x_l+0.1,0.95,"Preliminary")
Label_prelim.SetNDC()
...
if len(other_scans) >= 3:
        legend = ROOT.TLegend(0.65, 0.13, 0.96, 0.92, '', 'NBNDC')
        #legend.SetNColumns(2)

legend.AddEntry(main_scan['func'], args.main_label, 'L')
#legend.AddEntry(main_scan['func'], value_dict['main']+" "+args.main_label, 'L')
for i, other in enumerate(other_scans):
