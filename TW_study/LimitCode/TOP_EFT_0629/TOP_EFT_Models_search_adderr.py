...
class TopEFT_TWTT(PhysicsModel):
    def __init__(self, TW_AinttoAsm, TW_AbsmtoAsm, TW_AsmnlomlotoAlo, TT_AinttoAsm, TT_AbsmtoAsm, TT_AsmnnlomnlotoAnlo, TT_AinttoAsm_err, TT_AsmnnlomnlotoAnlo_err):
        self.poiMap = []
        self.pois = {}
        self.verbose = False
        self.TW_AinttoAsm         = float(TW_AinttoAsm)
        ...
        self.TT_AinttoAsm_err     = float(TT_AinttoAsm_err)
        self.TT_AsmnnlomnlotoAnlo_err = float(TT_AsmnnlomnlotoAnlo_err)
    def setModelBuilder(self, modelBuilder):
        PhysicsModel.setModelBuilder(self,modelBuilder)
        self.modelBuilder.doModelBOnly = False
        ####################### add fit parameters ###############################
        self.modelBuilder.doVar("TT_AinttoAsm[%f,%f,%f]"%(self.TT_AinttoAsm,self.TT_AinttoAsm-self.TT_AinttoAsm_err,self.TT_AinttoAsm+self.TT_AinttoAsm_err))
        self.modelBuilder.doVar("TT_AsmnnlomnlotoAnlo[%f,%f,%f]"%(self.TT_AsmnnlomnlotoAnlo,self.TT_AsmnnlomnlotoAnlo-self.TT_AsmnnlomnlotoAnlo_err,self.TT_AsmnnlomnlotoAnlo+self.TT_AsmnnlomnlotoAnlo_err))
    ...
    def doParametersOfInterest(self):
        ...
        self.modelBuilder.factory_( "expr::TT_s_func(\"(1)+(@0)*(@1) + pow(@0,2)*%f + (@2) \", r, TT_AinttoAsm,TT_AsmnnlomnlotoAnlo)"%(self.TT_AbsmtoAsm))
...
Model_Ctg  =TopEFT_TWTT( TW_AinttoAsm=6.65*1.27*0.5/55.24,TW_AbsmtoAsm=4.99*1.06*0.25/55.24,TW_AsmnlomlotoAlo=(71.7-55.24)/55.24,TT_AinttoAsm=405.66*0.5/730.7,TT_AbsmtoAsm=94.18*0.25/730.7,TT_AsmnnlomnlotoAnlo=(831.76-730.7)/730.7,TT_AinttoAsm_err=0.12*405.66*0.5/730.7,TT_AsmnnlomnlotoAnlo_err=0.063*(831.76-730.7)/730.7)
