...
def draw_plots(chan, date, dir, stack,h_mc,h_data, g_stat_sys ,out_name, xaxis_name, ratio_Style, log_hist, h_FCNC_tug, h_FCNC_tcg, EFT_dict):
    stat_style=3004
    stat_sys_style=3005
    stat_color=ROOT.TColor.GetColor("#000000")
    stat_sys_color=ROOT.TColor.GetColor("#262626")
    can_x_min=50
    can_x_max=865
    can_y_min=50
    can_y_max=780
...
#    pad2=ROOT.TPad("pad2", "pad2", 0, 0.0, 1, 0.3 , 0)#used for the ratio plot
    ...
    if Channel != "emu" and out_name == 'H_Limit_N_jet_bjet':
       ...
       nbin_x=3
       #y_max=0.6*y_max
       y_max=2*h_data.GetBinContent(4)
    if Channel == "emu" and out_name == 'H_Limit_N_jet_bjet':
       x_min=1
       nbin_x=4
       #y_max=0.6*y_max
    ...
    if h_data.GetBinWidth(1)%1 != 0:  
        dummy.GetYaxis().SetTitle('Events / %.1f'%(h_data.GetBinWidth(1)))
        if "GeV" in xaxis_name:dummy.GetYaxis().SetTitle('Events / %.1f GeV'%(h_data.GetBinWidth(1)))
    else:
        dummy.GetYaxis().SetTitle('Events / %.0f'%(h_data.GetBinWidth(1)))
        if "GeV" in xaxis_name:dummy.GetYaxis().SetTitle('Events / %.0f GeV'%(h_data.GetBinWidth(1)))
     
    if out_name == "H_Limit_N_jet_bjet": 
        dummy.GetYaxis().SetTitle('Events / bin')
        #dummy.GetYaxis().SetMaxDigits(4)
    #dummy.GetYaxis().SetNdivisions(405)
    dummy.GetXaxis().SetLabelSize(0)
    if out_name in ["H_MLP_1jet_0bjet_comb","H_MLP_1jet_1bjet_comb"]:
    ...
    h_mc.SetFillStyle(stat_style)
    h_mc.SetFillColor(stat_color)
    h_mc.SetLineColor(ROOT.kBlack)
    #h_mc.SetFillColorAlpha(ROOT.kOrange-3,1.0)
    #g_stat_sys.SetFillColorAlpha(ROOT.kBlue-3,1.0)
    g_stat_sys.SetFillStyle(stat_sys_style)
    g_stat_sys.SetFillColor(stat_sys_color)
    g_stat_sys.SetLineColor(ROOT.kBlack)
    g_stat_sys.Draw("2p")
    h_mc.Draw("sames:e2")
    ...
    h_Cphiq=ROOT.TH1D()
    if EFT_dict:
        for ieft in EFT_dict:
            if "Ctg" in ieft:
                h_Ctg=EFT_dict[ieft].Clone(ieft)
                h_Ctg.SetLineWidth(2)
                h_Ctg.SetLineColor(ROOT.TColor.GetColor("#0000ff")) 
                h_Ctg.Draw("same:hist")
            elif "Ctw" in ieft:
                h_Ctw=EFT_dict[ieft].Clone(ieft)
                h_Ctw.SetLineWidth(2)
                h_Ctw.SetLineColor(ROOT.TColor.GetColor("#008000")) 
                h_Ctw.Draw("same:hist")
            elif "Cphiq" in ieft:
                h_Cphiq=EFT_dict[ieft].Clone(ieft)
                h_Cphiq.SetLineWidth(2)
                h_Cphiq.SetLineColor(ROOT.TColor.GetColor("#ff00ff")) 
                h_Cphiq.Draw("same:hist")
    if remove_data==False:
        g_data_clone.Draw("pZ0")
    ...
    if Do_FCNC_Study:
        ...
        #legend.AddEntry(h_tmp_FCNC_tug,'C_{uG}(2.0)','l')#sqrt(10/16.7*0.324*0.324*1.27) = 2.119
        legend.AddEntry(h_tmp_FCNC_tug,'C_{uG}#font[22]{/}#Lambda^{2} = 2.0 TeV^{-2}','l')#sqrt(10/16.7*0.324*0.324*1.27) = 2.119
        legend.AddEntry(h_tmp_ttbar,'t#bar{t}','f')
        #legend.AddEntry(h_tmp_FCNC_tcg,'tcg(%d pb)'%(FCNC_XS),'l')
        #legend.AddEntry(h_tmp_FCNC_tcg,'C_{cG}(4.0)','l')#sqrt(10/4.57*0.324*0.324*1.27) =4.051
        legend.AddEntry(h_tmp_FCNC_tcg,'C_{cG}#font[22]{/}#Lambda^{2} = 4.0 TeV^{-2}','l')#sqrt(10/4.57*0.324*0.324*1.27) =4.051
        legend.AddEntry(h_tmp_DY,'DY','f')
        legend.AddEntry(h_tmp_uncertainty,'Stat.','f')
        legend.AddEntry(h_tmp_other,'Others','f')
        legend.AddEntry(h_tmp_all_uncertainty,'Stat. #oplus syst.','f')
        legend.AddEntry(h_tmp_ss,'Nonprompt','f')
    if EFT_dict:
        sf_Ctg=h_Ctg.GetName().split("_")[-1]
        sf_Ctw=h_Ctw.GetName().split("_")[-1]
        sf_Cphiq=h_Cphiq.GetName().split("_")[-1]
        legend.AddEntry(h_tmp_ST,"tW",'f')
        legend.AddEntry(h_Cphiq,'C_{#phiq}^{(3)}#font[22]{/}#Lambda^{2} = %.1f TeV^{-2}'%(float(sf_Cphiq)),'l')
        legend.AddEntry(h_tmp_ttbar,'t#bar{t}','f')
        legend.AddEntry(h_Ctg,'C_{tG}#font[22]{/}#Lambda^{2} = %.1f TeV^{-2}'%(float(sf_Ctg)),'l')
        legend.AddEntry(h_tmp_DY,'DY','f')
        legend.AddEntry(h_Ctw,'C_{tW}#font[22]{/}#Lambda^{2} = %.1f TeV^{-2}'%(float(sf_Ctw)),'l')
        legend.AddEntry(h_tmp_other,'Others','f')
        legend.AddEntry(h_tmp_uncertainty,'Stat.','f')
        legend.AddEntry(h_tmp_ss,'Nonprompt','f')
        legend.AddEntry(h_tmp_all_uncertainty,'Stat. #oplus syst.','f')
    if EFT_dict=={} and Do_FCNC_Study==False:
        legend.AddEntry(h_tmp_ST,"tW",'f')
        legend.AddEntry(h_tmp_ttbar,'t#bar{t}','f')
        legend.AddEntry(h_tmp_DY,'DY','f')
        legend.AddEntry(h_tmp_other,'Others','f')
...
########################################
    ...
    cms_text_sixe=0.08
    label_x_l=0.17
    Label_cms = ROOT.TLatex(label_x_l,0.8,label_cms)
    Label_cms.SetNDC()
    Label_cms.SetTextSize(cms_text_sixe)
    Label_cms.SetTextFont(61)
    Label_cms.Draw()
    Label_prelim = ROOT.TLatex(label_x_l,0.73,"Preliminary")
    if for_paper_style is False:
        Label_prelim.SetNDC()
        Label_prelim.SetTextSize(cms_text_sixe*0.8)
        Label_prelim.SetTextFont(51)
        Label_prelim.Draw()

##############################################
    Label_lumi_x_min=0.71
    Label_lumi_y_min=0.92
    if out_name == "H_njet_bjet":
        Label_lumi_x_min=0.78
        Label_lumi_y_min=0.93
    Label_lumi = ROOT.TLatex(Label_lumi_x_min,Label_lumi_y_min,"%.1f fb^{-1} (13 TeV)"%(float(Lumi)/1000))
    Label_lumi.SetTextSize(0.04/0.7)
    Label_lumi.SetNDC()
    Label_lumi.SetTextFont(42)
    Label_lumi.Draw()
    Label_chan=ROOT.TPaveText(label_x_l,0.65, 0.35,0.63,"blNDC")
    Label_chan.SetTextFont(42)##just like text
    Label_chan.SetTextSize(0.07)
    if for_paper_style is True:
        Label_chan=ROOT.TPaveText(label_x_l,0.7, 0.35,0.63,"blNDC")
    ...
    Label_chan.Draw()
    Label_region=ROOT.TPaveText(label_x_l,0.57, 0.33,0.55,"blNDC")
    ...
    Label_region.SetTextSize(0.05)
    if "0j0b" in out_name or "0jet_0bjet" in out_name: 
        Label_region.AddText('(0-jet,0-tag)')
    elif "1j0b" in out_name or "1jet_0bjet" in out_name: 
        Label_region.AddText('(1-jet,0-tag)')
    elif "1j1b" in out_name or "1jet_1bjet" in out_name: 
        Label_region.AddText('(1-jet,1-tag)')
    elif "2j1b" in out_name or "2jet_1bjet" in out_name: 
        Label_region.AddText('(2-jets,1-tag)')
    elif "2j2b" in out_name or "2jet_2bjet" in out_name: 
        Label_region.AddText('(#geq2-jets,2-tags)')
    elif "xj1b" in out_name or "xjet_1bjet" in out_name: 
        Label_region.AddText('(#font[12]{n}-jets,1-tag)')
    Label_region.Draw()
    Label_pvn=ROOT.TPaveText(0.15,0.58, 0.32,0.56,"blNDC")
    ...
    Label_pvn.SetTextSize(0.05)
#    Label_pvn.AddText('Nvtx(25-30)')
    Label_pvn.AddText('TT_TuneEE5C')
#    Label_pvn.Draw()
##### for ratio label #######################
    ...
    label_ratio_x_min=0.15
    label_ratio_x_max=0.4
    ...
    if remove_data==False and out_name not in ["H_njet_bjet","H_Limit_N_jet_bjet","H_xjet_1bjet"] and "MLP" not in out_name:
        label_ratio.Draw()
    ######################################
    pad2.cd()
    ratio_y_min=-0.5
...
#    dummy_ratio.GetYaxis().SetLabelOffset(0.01)
    ...
    if out_name in ["H_MLP_1jet_0bjet_comb","H_MLP_1jet_1bjet_comb"]:
        ...
        #n_div = int((x_max-x_min)/0.1) + 100*5
        #dummy_ratio.GetXaxis().SetNdivisions(n_div)
    #dummy_ratio.GetYaxis().SetNdivisions(504)#4 is first, 5*100 is second
    dummy_ratio.GetYaxis().SetNdivisions(204)#4 is first, 2*100 is second
    ...
    g_mc_stat.Draw("2p")
    if EFT_dict:
        h_Ctg_ratio=h_Ctg.Clone(h_Ctg.GetName()+"_ratio")
        h_Ctw_ratio=h_Ctw.Clone(h_Ctw.GetName()+"_ratio")
        h_Cphiq_ratio=h_Cphiq.Clone(h_Cphiq.GetName()+"_ratio")
        h_Ctg_ratio.Divide(h_mc)
        h_Ctw_ratio.Divide(h_mc)
        h_Cphiq_ratio.Divide(h_mc)
        h_Ctg_ratio.Draw("same:hist")
        h_Ctw_ratio.Draw("same:hist")
        h_Cphiq_ratio.Draw("same:hist")
    if remove_data==False:
        g_ratio.Draw("pZ0")
    dummy_ratio.Draw("AXISSAME")
    #####################################
    if EFT_dict:
        out_name=out_name+"_"+h_Ctg.GetName()+"_"+h_Ctw.GetName()+"_"+h_Cphiq.GetName()
    canvas.Print('%s/%s/%s.png'%(dir,date,out_name))    
    canvas.Print('%s/%s/%s.pdf'%(dir,date,out_name))    
    del canvas
    gc.collect()
...
#        for nsys in Dicit[hname]:
        ...
        Label_chan.SetTextSize(0.1)
        str_region=""
        if "0j0b" in hname or "0jet_0bjet" in hname: 
            str_region='(0jet,0bjet)'
        elif "1j0b"in hname or "1jet_0bjet" in hname: 
            str_region='(1jet,0bjet)'
        elif "1j1b"in hname or "1jet_1bjet" in hname: 
            str_region='(1jet,1bjet)'
        elif "2j1b"in hname or "2jet_1bjet" in hname: 
            str_region='(2jet,1bjet)'
        elif "2j2b"in hname or "2jet_2bjet" in hname: 
            str_region='(#geq2jet,2bjet)'
        if "ee" in Channel:
            if "Sample_TT" in sys_name:
                Label_chan.AddText ('t#bar{t}(ee)'+str_region)
            elif "Sample_ST_tW" in sys_name:
                Label_chan.AddText ('tW(ee)'+str_region)
            else:
                Label_chan.AddText ('ee'+str_region)
        elif "emu" in Channel:
            if "Sample_TT" in sys_name:
                Label_chan.AddText ('t#bar{t}(e#mu)'+str_region)
            elif "Sample_ST_tW" in sys_name:
                Label_chan.AddText ('tW(e#mu)'+str_region)
            else:
                Label_chan.AddText ('e#mu'+str_region)
        elif "mumu" in Channel:
            if "Sample_TT" in sys_name:
                Label_chan.AddText ('t#bar{t}(#mu#mu)'+str_region)
            elif "Sample_ST_tW" in sys_name:
                Label_chan.AddText ('tW(#mu#mu)'+str_region)
            else:
                Label_chan.AddText ('#mu#mu'+str_region)
        Label_chan.Draw()
        pad2.cd()
        legend.Draw()
...
def Draw_all_system_ratio_v2(dir, date, Dicit, color, dir_hist, dir_system_name, sys_list, sys_name, shape_name):
    
    for hname in Dicit:
        canvas = ROOT.TCanvas('canvas_%s'%(hname),'',50,50,865,780)
        canvas.cd()
        canvas.cd()
        pad1=ROOT.TPad("pad1", "pad1", 0  , 0.4, 0.8, 1  , 0)#used for the hist plot
        padr=ROOT.TPad("padr", "padr", 0  , 0  , 0.8, 0.4, 0)#used for the hist plot
        padl=ROOT.TPad("padl", "padl", 0.8, 0  , 1  , 1  , 0)#used for the ratio plot
        pad1.Draw()
        padr.Draw()
        padl.Draw() 
        pad1.SetTickx()
        ...
        padr.SetTopMargin(0.01)
        padr.SetLeftMargin(0.14)
        padr.SetRightMargin(0.024)
        padl.SetTopMargin(0.08)
        padl.SetBottomMargin(0.04)
        padl.SetRightMargin(0.15)
        padr.cd()
        h_mc_nominal=Dicit[hname]["nominal"].Clone("%s_nominal"%(hname))
        x_min=h_mc_nominal.GetXaxis().GetXmin()
        x_max=h_mc_nominal.GetXaxis().GetXmax()
        y_min=0
        y_max=1.5*h_mc_nominal.GetBinContent(h_mc_nominal.GetMaximumBin())
        ...
        dummy_ratio.GetYaxis().SetMoreLogLabels()
        dummy_ratio.GetYaxis().SetNoExponent()  
        dummy_ratio.GetXaxis().SetTitleSize(0.07)
        dummy_ratio.GetYaxis().SetTitleSize(0.07)
        dummy_ratio.GetXaxis().SetTitleFont(42)
        dummy_ratio.GetYaxis().SetTitleFont(42)
        dummy_ratio.GetXaxis().SetTickLength(0.05)
        dummy_ratio.GetYaxis().SetTickLength(0.05)
        dummy_ratio.GetXaxis().SetLabelSize(0.024/0.3)
        dummy_ratio.GetYaxis().SetLabelSize(0.024/0.3)
        dummy_ratio.GetXaxis().SetLabelOffset(0.01)
        dummy_ratio.GetYaxis().SetLabelOffset(0.01)
        dummy_ratio.GetYaxis().SetTitleOffset(1)
        dummy_ratio.GetXaxis().SetTitleOffset(1.2)
        dummy_ratio.GetYaxis().SetNdivisions(510)
        dummy_ratio.Draw()
        legend = ROOT.TLegend(0.01,0.02,0.95,0.92)
...
#        for nsys in Dicit[hname]:
        pad1.cd()
        dummy = ROOT.TH2F("dummy_%s"%(hname),"",nbin,x_min,x_max,1,y_min,y_max)
        dummy.SetStats(ROOT.kFALSE)
        dummy.GetYaxis().SetTitle('Events')
        ...
        dummy.GetXaxis().SetTickLength(0.05)
        dummy.GetYaxis().SetTickLength(0.05)
        dummy.GetXaxis().SetLabelSize(0)
        dummy.GetYaxis().SetLabelSize(0.012/0.3)
        dummy.GetXaxis().SetLabelOffset(0.01)
        dummy.GetYaxis().SetLabelOffset(0.01)
        dummy.GetYaxis().SetTitleOffset(1.3)
        dummy.GetXaxis().SetTitleOffset(0)
        dummy.GetYaxis().SetNdivisions(510)
        dummy.Draw()
        h_mc_nominal.SetLineColor(ROOT.kBlack)
        h_mc_nominal.SetLineWidth(2)
        h_mc_nominal.SetLineStyle(1)
        h_mc_nominal.Draw("same:pe")
        legend.AddEntry(h_mc_nominal,"Nominal",'l')
        for nsys in sys_list:
            if Channel=="emu"     and "UnclusteredEn" in nsys:continue
            elif Channel=="ee"    and "Muon" in nsys:continue
            ...
            #if sys_name not in nsys and "nominal" not in nsys:continue
            h_mc=Dicit[hname][nsys].Clone("ratio_%s_%s"%(hname,nsys))
            h_mc_sys=Dicit[hname][nsys].Clone("sys_%s_%s"%(hname,nsys))
            h_mc.Divide(h_mc_nominal)
            h_mc.SetLineWidth(line_width)
            h_mc_sys.SetLineWidth(line_width)
            if "Sample_" in nsys and "CR" not in nsys:
                cname=nsys.split("Sample_")[-1]
                h_mc.SetLineColor(color[cname].color)
                h_mc_sys.SetLineColor(color[cname].color)
            elif "CR" in nsys:
                h_mc.SetLineColor(color["TT_QCDbasedCRTune_erdON"].color)
                h_mc_sys.SetLineColor(color["TT_QCDbasedCRTune_erdON"].color)
            elif "isr" in nsys:
                h_mc.SetLineColor(color["TT_isrdown"].color)
                h_mc_sys.SetLineColor(color["TT_isrdown"].color)
            elif "fsr" in nsys:
                h_mc.SetLineColor(color["TT_fsrdown"].color)
                h_mc_sys.SetLineColor(color["TT_fsrdown"].color)
            else:
                h_mc.SetLineColor(color[nsys].color)
                h_mc_sys.SetLineColor(color[nsys].color)
            if "down" in nsys or "Down" in nsys or "DOWN" in nsys:
                h_mc.SetLineStyle(2)
                h_mc_sys.SetLineStyle(2)
                cname=nsys
                cname.replace("down","up")
                cname.replace("Down","Up")
                cname.replace("DOWN","UP")
            ...
            legend.AddEntry(h_mc,nlegend,'l')
            dicit_dummy.append(h_mc)
            dicit_dummy_sys.append(h_mc_sys)
        padr.cd()
        dummy_ratio.Draw("AXISSAME")
        pad1.cd()
        dummy.Draw("AXISSAME")
        ...
        Label_cms_prelim.SetShadowColor(10)
        Label_cms_prelim.Draw()
        Label_lumi = ROOT.TPaveText(0.73,0.92,0.98,0.97,"NDC")
        Label_lumi.SetLineColor(10)
        Label_lumi.SetFillColor(10)
        Label_lumi.SetTextSize(0.05)
        Label_lumi.SetTextAlign(12)
        ...
        padl.cd()
        legend.Draw()
        canvas.Print('%s/%s/H_%s_%s.png'%(dir,date,shape_name,hname))    
        del canvas
        gc.collect()
...
def Envelope(*args):
    ...
    if N==0:
        print" Error no input"
        return
    h_up  =args[0].Clone("%s_envelope_up"%(args[0].GetName()))
    h_down=args[0].Clone("%s_envelope_down"%(args[0].GetName()))
    for ibin in range(1,h_up.GetNbinsX()+1):
        value=0
        error=0
        tmp_list=[]
        for ih in range(0,N):
            tmp_list.append(args[ih].GetBinContent(ibin))
        up_index  =numpy.argmax(tmp_list)
        down_index=numpy.argmin(tmp_list)
        h_up.SetBinContent(ibin,args[up_index].GetBinContent(ibin))
        h_up.SetBinError  (ibin,args[up_index].GetBinError  (ibin))
        h_down.SetBinContent(ibin,args[down_index].GetBinContent(ibin))
        h_down.SetBinError  (ibin,args[down_index].GetBinError  (ibin))
    return [h_up,h_down]
...
%(bgk_TTTo2L2Nu_scale_up)s                  & %(bgk_TTTo2L2Nu_scale_up_all)s                   & %(bgk_TTTo2L2Nu_scale_up_1j0t)s                   & %(bgk_TTTo2L2Nu_scale_up_1j1t)s                   & %(bgk_TTTo2L2Nu_scale_up_2j1t)s                  & %(bgk_TTTo2L2Nu_scale_up_2j2t)s                   \\\\ \hline 
%(bgk_TTTo2L2Nu_scale_down)s                & %(bgk_TTTo2L2Nu_scale_down_all)s                 & %(bgk_TTTo2L2Nu_scale_down_1j0t)s                 & %(bgk_TTTo2L2Nu_scale_down_1j1t)s                 & %(bgk_TTTo2L2Nu_scale_down_2j1t)s                & %(bgk_TTTo2L2Nu_scale_down_2j2t)s                 \\\\ \hline 
...
%(Sample_DY_PDF_Up)s                        & %(Sample_DY_PDF_Up_all)s                         & %(Sample_DY_PDF_Up_1j0t)s                         & %(Sample_DY_PDF_Up_1j1t)s                         & %(Sample_DY_PDF_Up_2j1t)s                        & %(Sample_DY_PDF_Up_2j2t)s                         \\\\ \hline    
%(Sample_DY_PDF_Down)s                      & %(Sample_DY_PDF_Down_all)s                       & %(Sample_DY_PDF_Down_1j0t)s                       & %(Sample_DY_PDF_Down_1j1t)s                       & %(Sample_DY_PDF_Down_2j1t)s                      & %(Sample_DY_PDF_Down_2j2t)s                       \\\\ \hline    
%(Sample_DY_QCD_Up)s                        & %(Sample_DY_QCD_Up_all)s                         & %(Sample_DY_QCD_Up_1j0t)s                         & %(Sample_DY_QCD_Up_1j1t)s                         & %(Sample_DY_QCD_Up_2j1t)s                        & %(Sample_DY_QCD_Up_2j2t)s                         \\\\ \hline    
%(Sample_DY_QCD_Down)s                      & %(Sample_DY_QCD_Down_all)s                       & %(Sample_DY_QCD_Down_1j0t)s                       & %(Sample_DY_QCD_Down_1j1t)s                       & %(Sample_DY_QCD_Down_2j1t)s                      & %(Sample_DY_QCD_Down_2j2t)s                       \\\\ \hline    
...
%(Sample_TT_CR_Up)s                      & %(Sample_TT_CR_Up_all)s                             & %(Sample_TT_CR_Up_1j0t)s                          & %(Sample_TT_CR_Up_1j1t)s                          & %(Sample_TT_CR_Up_2j1t)s                          & %(Sample_TT_CR_Up_2j2t)s                         \\\\ \hline        
%(Sample_TT_CR_Down)s                    & %(Sample_TT_CR_Down_all)s                           & %(Sample_TT_CR_Down_1j0t)s                        & %(Sample_TT_CR_Down_1j1t)s                        & %(Sample_TT_CR_Down_2j1t)s                       & %(Sample_TT_CR_Down_2j2t)s                        \\\\ \hline        
...
%(Sample_TT_QCD_Up)s                        & %(Sample_TT_QCD_Up_all)s                         & %(Sample_TT_QCD_Up_1j0t)s                         & %(Sample_TT_QCD_Up_1j1t)s                         & %(Sample_TT_QCD_Up_2j1t)s                        & %(Sample_TT_QCD_Up_2j2t)s                         \\\\ \hline    
%(Sample_TT_QCD_Down)s                      & %(Sample_TT_QCD_Down_all)s                       & %(Sample_TT_QCD_Down_1j0t)s                       & %(Sample_TT_QCD_Down_1j1t)s                       & %(Sample_TT_QCD_Down_2j1t)s                      & %(Sample_TT_QCD_Down_2j2t)s                       \\\\ \hline    
...
%(bgk_TTTo2L2Nu_scale_up)s                  & %(bgk_TTTo2L2Nu_scale_up_all)s                   & %(bgk_TTTo2L2Nu_scale_up_1j1t)s                   & %(bgk_TTTo2L2Nu_scale_up_2j1t)s                  & %(bgk_TTTo2L2Nu_scale_up_2j2t)s                   \\\\ \hline 
%(bgk_TTTo2L2Nu_scale_down)s                & %(bgk_TTTo2L2Nu_scale_down_all)s                 & %(bgk_TTTo2L2Nu_scale_down_1j1t)s                 & %(bgk_TTTo2L2Nu_scale_down_2j1t)s                & %(bgk_TTTo2L2Nu_scale_down_2j2t)s                 \\\\ \hline 
%(bgk_DYJetsToLL_M10to50_amc_DYJetsToLL_M50_amc_scale_up)s  & %(bgk_DYJetsToLL_M10to50_amc_DYJetsToLL_M50_amc_scale_up_all)s   & %(bgk_DYJetsToLL_M10to50_amc_DYJetsToLL_M50_amc_scale_up_1j1t)s   & %(bgk_DYJetsToLL_M10to50_amc_DYJetsToLL_M50_amc_scale_up_2j1t)s  & %(bgk_DYJetsToLL_M10to50_amc_DYJetsToLL_M50_amc_scale_up_2j2t)s   \\\\ \hline 
%(bgk_DYJetsToLL_M10to50_amc_DYJetsToLL_M50_amc_scale_down)s& %(bgk_DYJetsToLL_M10to50_amc_DYJetsToLL_M50_amc_scale_down_all)s & %(bgk_DYJetsToLL_M10to50_amc_DYJetsToLL_M50_amc_scale_down_1j1t)s & %(bgk_DYJetsToLL_M10to50_amc_DYJetsToLL_M50_amc_scale_down_2j1t)s& %(bgk_DYJetsToLL_M10to50_amc_DYJetsToLL_M50_amc_scale_down_2j2t)s \\\\ \hline 
...
%(Sample_TT_CR_Up)s                        & %(Sample_TT_CR_Up_all)s                           & %(Sample_TT_CR_Up_1j1t)s                          & %(Sample_TT_CR_Up_2j1t)s                         & %(Sample_TT_CR_Up_2j2t)s                          \\\\ \hline        
%(Sample_TT_CR_Down)s                      & %(Sample_TT_CR_Down_all)s                         & %(Sample_TT_CR_Down_1j1t)s                        & %(Sample_TT_CR_Down_2j1t)s                       & %(Sample_TT_CR_Down_2j2t)s                        \\\\ \hline        
...
%(Sample_TT_QCD_Up)s                        & %(Sample_TT_QCD_Up_all)s                         & %(Sample_TT_QCD_Up_1j1t)s                         & %(Sample_TT_QCD_Up_2j1t)s                        & %(Sample_TT_QCD_Up_2j2t)s                       \\\\ \hline    
%(Sample_TT_QCD_Down)s                      & %(Sample_TT_QCD_Down_all)s                       & %(Sample_TT_QCD_Down_1j1t)s                       & %(Sample_TT_QCD_Down_2j1t)s                      & %(Sample_TT_QCD_Down_2j2t)s                       \\\\ \hline    
...
class sys_uncert_object:
    def __init__(self, name, sys_list, file_in, mc_sample, sys_sample, hist_dir):
        ...
        self.all_hist={}
        self.xsection_uncer={}
        self.xsection_uncer["ST"]            =0.10##for FCNC      
        self.xsection_uncer["ST_anti"]       =0.10##for FCNC
            ...
            for nsys in self.sys_list:
                ...
                for sname in self.mc_sample:
                    ...
                    if "lumi" in nsys or "bgk" in nsys :
                        ...
                        if "luminosity_up" in nsys:
                            self.all_hist[hname][nsys][sname].Scale(1.025)         
                        elif "luminosity_down" in nsys:
                            self.all_hist[hname][nsys][sname].Scale(0.975)         
                        elif "bgk" in nsys:
                            ########################## for emu DY 15% uncertainty for 0j0b and 1j0b and all #####################################################
                            if Channel=="emu" and "DYJets" in nsys and "DYJets" in sname: 
                                if "1jet_1bjet" in hname or "1j1b" in hname or "2jet_1bjet" in hname or "2j1b" in hname or "2jet_2bjet" in hname or "2j2b" in hname or "xjet_1bjet" in hname or "xj1b" in hname:
                                    if "up" in nsys or "Up" in nsys:
                                        self.all_hist[hname][nsys][sname].Scale(1.5)         
                                    elif "down" in nsys or "Down" in nsys:
                                        self.all_hist[hname][nsys][sname].Scale(0.5) 
                                elif hname in ['H_njet_bjet','H_Limit_N_jet_bjet']:
                                    scale_value=1
                                    if "up" in nsys or "Up" in nsys:
                                        for ii in range(1,self.all_hist[hname][nsys][sname].GetNbinsX()+1):
                                            if ii<=2  :scale_value=1.15 ##bin 0 and bin 1 is 0j0b and 1j0b
                                            else      :scale_value=1.5
                                            self.all_hist[hname][nsys][sname].SetBinContent(ii,scale_value*self.all_hist[hname][nsys][sname].GetBinContent(ii))         
                                            self.all_hist[hname][nsys][sname].SetBinError  (ii,scale_value*self.all_hist[hname][nsys][sname].GetBinError  (ii))         
                                    elif "down" in nsys or "Down" in nsys:
                                        for ii in range(1,self.all_hist[hname][nsys][sname].GetNbinsX()+1):
                                            if ii<=2  :scale_value=0.85
                                            else      :scale_value=0.5
                                            self.all_hist[hname][nsys][sname].SetBinContent(ii,scale_value*self.all_hist[hname][nsys][sname].GetBinContent(ii))         
                                            self.all_hist[hname][nsys][sname].SetBinError  (ii,scale_value*self.all_hist[hname][nsys][sname].GetBinError  (ii))         
                                else:    
                                    if "up" in nsys or "Up" in nsys:
                                        self.all_hist[hname][nsys][sname].Scale(1.15)         
                                    elif "down" in nsys or "Down" in nsys:
                                        self.all_hist[hname][nsys][sname].Scale(0.85) 
                            else: 
                                self.all_hist[hname][nsys][sname].Scale(self.Xsection_uncert(nsys,sname))         
                        else: print "something wrong ?"   
                     ############### CR envelope ##################
                    elif "Sample_TT_CR" in nsys: ##for CR envelope
                        if "TTTo2L2Nu" in sname:
                            ...
                            if "Up" in nsys or "up" in nsys:
                                self.all_hist[hname][nsys][sname]=Envelope(h_tmp1,h_tmp2,h_tmp3)[0]
                            elif "Down" in nsys or "down" in nsys:
                                self.all_hist[hname][nsys][sname]=Envelope(h_tmp1,h_tmp2,h_tmp3)[1]
                            else:
                                print "wrong nsys name?"
                        else:
                     ...
                     ############# TT, TW ISR, FSR ################################
                        ...
                        if "TTTo2L2Nu" in sname:
                            ...
                            tmp_scale=float(self.lumi)/float(self.mc_sample[sname].lumi)
                            self.all_hist[hname][nsys][sname].Scale(tmp_scale)         
                     ############# DY PDF, Qscale for all and 0j0b, 1j0b ################################
                    elif "Sample" in nsys and "DY" in nsys:
                        if "DYJetsToLL_M50_amc" in sname:
                            if "1jet_1bjet" in hname or "1j1b" in hname or "2jet_1bjet" in hname or "2j1b" in hname or "2jet_2bjet" in hname or "2j2b" in hname or "xjet_1bjet" in hname or "xj1b" in hname:
                                self.all_hist[hname][nsys][sname]=self.file_in["nominal"].tfile.Get("nominal"+"__"+sname+"__"+hname)
                                tmp_scale=float(self.lumi)/float(self.mc_sample[sname].lumi)
                                self.all_hist[hname][nsys][sname].Scale(tmp_scale)         
                            elif hname in ['H_njet_bjet','H_Limit_N_jet_bjet']:
                                tmp_nominal=self.file_in["nominal"].tfile.Get("nominal"+"__"+sname+"__"+hname)
                                tmp_nominal_scale=float(self.lumi)/float(self.mc_sample[sname].lumi)
                                tmp_nominal.Scale(tmp_nominal_scale)         
                                tmp_sys=self.file_in[nsys.split("Sample_")[-1]].tfile.Get(nsys.split("Sample_")[-1]+"__"+sname+"__"+hname)
                                tmp_sys_scale=float(self.lumi)/float(self.sys_sample[nsys.split("Sample_")[-1]].lumi)
                                tmp_sys.Scale(tmp_sys_scale)         
                                tmp_nominal.SetBinContent(1,tmp_sys.GetBinContent(1))
                                tmp_nominal.SetBinError  (1,tmp_sys.GetBinError  (1))
                                tmp_nominal.SetBinContent(2,tmp_sys.GetBinContent(2))
                                tmp_nominal.SetBinError  (2,tmp_sys.GetBinError  (2))
                                self.all_hist[hname][nsys][sname]=tmp_nominal         
                            else:
                     ...
                     ############# TT, TW modeling ################################
                        ...
                        #    print "nsys %s:, hname %s, scale %f"%(nsys,hname,tmp_scale)
                            ...
                            tmp_scale=float(self.lumi)/float(self.mc_sample[sname].lumi)
                            self.all_hist[hname][nsys][sname].Scale(tmp_scale)         
                     ############# FCNC modeling uncertainty ################################
                    elif "Sample_FCNC_" in nsys:
                        if "FCNC_ST_tug" in sname:
                            self.all_hist[hname][nsys][sname]=self.file_in[nsys.split("Sample_")[-1]].tfile.Get(nsys.split("Sample_")[-1]+"__"+sname+"__"+hname)
                            str_sys_sample_1=nsys.split("Sample_")[-1]
                            str_sys_sample  =str_sys_sample_1.replace("FCNC_","FCNC_tug_")
                            tmp_scale=float(self.lumi)/float(self.sys_sample[str_sys_sample].lumi)
                            self.all_hist[hname][nsys][sname].Scale(tmp_scale)         
                        elif "FCNC_ST_tcg" in sname:
                            self.all_hist[hname][nsys][sname]=self.file_in[nsys.split("Sample_")[-1]].tfile.Get(nsys.split("Sample_")[-1]+"__"+sname+"__"+hname)
                            str_sys_sample_1=nsys.split("Sample_")[-1]
                            str_sys_sample  =str_sys_sample_1.replace("FCNC_","FCNC_tcg_")
                            tmp_scale=float(self.lumi)/float(self.sys_sample[str_sys_sample].lumi)
                            self.all_hist[hname][nsys][sname].Scale(tmp_scale)         
                        else:
                            self.all_hist[hname][nsys][sname]=self.file_in["nominal"].tfile.Get("nominal"+"__"+sname+"__"+hname)
...
#Channel="emu"
Channel="mumu"
for_paper_style     =True ##plot style

Do_FCNC_Study       =False# False is for EFT study
remove_data         =False
do_same_sign        =True
remove_herwig       =True
do_system           =True
...
do_save_sys_hist    =True # save output for limit
do_plot_main        =True # main plot
draw_EFT_plot       =True # EFT plot
do_print_sys_region =True
do_draw_combine_MVA =False#not used
do_print_table      =False#not used
do_trigger_study    =False#not used
...
##################### QCD scale, FCNC sys sample ####################
#File_path="./ntuples/saved_hist/Step2_0818_QCD/"
#File_path_same_sign="./ntuples/saved_hist/Step2_SameSign_0818_QCD/"
#################### emu cut change  ####################
#File_path="./ntuples/saved_hist/Step2_0820_emu/"
#File_path_same_sign="./ntuples/saved_hist/Step2_SameSign_0820_emu/"
#################### PDF and QCD scale updated ####################
#File_path="./ntuples/saved_hist/Step2_0828_QCD_PDF/"
#File_path_same_sign="./ntuples/saved_hist/Step2_SameSign_0828_QCD_PDF/"
#################### xjet_1bjet added ####################
#File_path="./ntuples/saved_hist/Step2_1006_1bjet/"
#File_path_same_sign="./ntuples/saved_hist/Step2_SameSign_1006_1bjet/"
#################### MVA variable for xjet_1bjet added ####################
#################### update FSR/sqrt(2), top mass /6 ####################
#File_path="./ntuples/saved_hist/Step2_1013_xjet_1bjet/"
#File_path="./ntuples/saved_hist/Step2_20180425_FSR_check_odd_v/"##check FSR odd event
#File_path="./ntuples/saved_hist/Step2_20180425_FSR_check_even_v/"##check FSR even event
#################### using more TT sys sample events ####################
#File_path="./ntuples/saved_hist/Step2_20180518_MoreTTsys_events/"
#File_path_same_sign="./ntuples/saved_hist/Step2_SameSign_1013_xjet_1bjet/"
#################### using new MVA MLP ####################
#File_path="./ntuples/saved_hist/Step2_20180608_newMVA/"
#File_path_same_sign="./ntuples/saved_hist/Step2_SameSign_20180608_newMVA/"
#################### using new MVA MLP ####################
#File_path="./ntuples/saved_hist/Step2_20180609_newMVA/"
#File_path_same_sign="./ntuples/saved_hist/Step2_SameSign_20180609_newMVA/"
################### top pt reweighting ####################
File_path="./ntuples/saved_hist/Step2_20180623_top_pt_reweight/"
File_path_same_sign="./ntuples/saved_hist/Step2_SameSign_20180623_top_pt_reweight/"
...
############### for DY PDF uncertainty ########## 
        Files["DY_PDF_Up"              ]=file_object(File_path, Channel+"_DY_PDF_Up_"               ,"data_mc",Lumi,1,ROOT.TColor.GetColor("#ccff00"))
        Files["DY_PDF_Down"            ]=file_object(File_path, Channel+"_DY_PDF_Down_"             ,"data_mc",Lumi,1,ROOT.TColor.GetColor("#ccff00"))
############### for DY QCD uncertainty ########## 
        Files["DY_QCD_Up"              ]=file_object(File_path, Channel+"_DY_QCD_Up_"               ,"data_mc",Lumi,1,ROOT.TColor.GetColor("#999999"))
        Files["DY_QCD_Down"            ]=file_object(File_path, Channel+"_DY_QCD_Down_"             ,"data_mc",Lumi,1,ROOT.TColor.GetColor("#999999"))
#################################################
...
############### for TT QCD uncertainty ########## 
        Files["TT_QCD_Up"              ]=file_object(File_path, Channel+"_TT_QCD_Up_"               ,"data_mc",Lumi,1,ROOT.TColor.GetColor("#999999"))
        Files["TT_QCD_Down"            ]=file_object(File_path, Channel+"_TT_QCD_Down_"             ,"data_mc",Lumi,1,ROOT.TColor.GetColor("#999999"))
#################################################
        ...
        if Do_FCNC_Study:
            ############### for FCNC modeling uncertainty ########## 
            Files["FCNC_PS_Up"         ]=file_object(File_path, Channel+"_FCNC_PS_Up_"              ,"data_mc",Lumi,1,ROOT.TColor.GetColor("#008080"))
            Files["FCNC_PS_Down"       ]=file_object(File_path, Channel+"_FCNC_PS_Down_"            ,"data_mc",Lumi,1,ROOT.TColor.GetColor("#008080"))
            Files["FCNC_PDF_Up"        ]=file_object(File_path, Channel+"_FCNC_PDF_Up_"             ,"data_mc",Lumi,1,ROOT.TColor.GetColor("#ccff00"))
            Files["FCNC_PDF_Down"      ]=file_object(File_path, Channel+"_FCNC_PDF_Down_"           ,"data_mc",Lumi,1,ROOT.TColor.GetColor("#ccff00"))
            Files["FCNC_QCD_Up"        ]=file_object(File_path, Channel+"_FCNC_QCD_Up_"             ,"data_mc",Lumi,1,ROOT.TColor.GetColor("#bdb76b"))
            Files["FCNC_QCD_Down"      ]=file_object(File_path, Channel+"_FCNC_QCD_Down_"           ,"data_mc",Lumi,1,ROOT.TColor.GetColor("#bdb76b"))
        if do_same_sign:
...
############### for DY PDF uncertainty ########## 
            Files_same_sign["DY_PDF_Up"              ]=file_object(File_path_same_sign, Channel+"_DY_PDF_Up_"               ,"data_mc",Lumi,1,ROOT.TColor.GetColor("#ccff00"))
            Files_same_sign["DY_PDF_Down"            ]=file_object(File_path_same_sign, Channel+"_DY_PDF_Down_"             ,"data_mc",Lumi,1,ROOT.TColor.GetColor("#ccff00"))
############### for DY QCD uncertainty ########## 
            Files_same_sign["DY_QCD_Up"              ]=file_object(File_path_same_sign, Channel+"_DY_QCD_Up_"               ,"data_mc",Lumi,1,ROOT.TColor.GetColor("#999999"))
            Files_same_sign["DY_QCD_Down"            ]=file_object(File_path_same_sign, Channel+"_DY_QCD_Down_"             ,"data_mc",Lumi,1,ROOT.TColor.GetColor("#999999"))
#################################################
...
############### for same-sign TT QCD uncertainty ########## 
            Files_same_sign["TT_QCD_Up"              ]=file_object(File_path_same_sign, Channel+"_TT_QCD_Up_"               ,"data_mc",Lumi,1,ROOT.TColor.GetColor("#bdb76b"))
            Files_same_sign["TT_QCD_Down"            ]=file_object(File_path_same_sign, Channel+"_TT_QCD_Down_"             ,"data_mc",Lumi,1,ROOT.TColor.GetColor("#bdb76b"))
########    #####_same_sign####################################
            ...
            if Do_FCNC_Study:
                ############### for FCNC modeling uncertainty ########## 
                Files_same_sign["FCNC_PS_Up"         ]=file_object(File_path_same_sign, Channel+"_FCNC_PS_Up_"              ,"data_mc",Lumi,1,ROOT.TColor.GetColor("#008080"))
                Files_same_sign["FCNC_PS_Down"       ]=file_object(File_path_same_sign, Channel+"_FCNC_PS_Down_"            ,"data_mc",Lumi,1,ROOT.TColor.GetColor("#008080"))
                Files_same_sign["FCNC_PDF_Up"        ]=file_object(File_path_same_sign, Channel+"_FCNC_PDF_Up_"             ,"data_mc",Lumi,1,ROOT.TColor.GetColor("#ccff00"))
                Files_same_sign["FCNC_PDF_Down"      ]=file_object(File_path_same_sign, Channel+"_FCNC_PDF_Down_"           ,"data_mc",Lumi,1,ROOT.TColor.GetColor("#ccff00"))
                Files_same_sign["FCNC_QCD_Up"        ]=file_object(File_path_same_sign, Channel+"_FCNC_QCD_Up_"             ,"data_mc",Lumi,1,ROOT.TColor.GetColor("#bdb76b"))
                Files_same_sign["FCNC_QCD_Down"      ]=file_object(File_path_same_sign, Channel+"_FCNC_QCD_Down_"           ,"data_mc",Lumi,1,ROOT.TColor.GetColor("#bdb76b"))
...
################# check for different TTbar sample  ########################
#    MC_Sample["TT_inclusive"          ]=mc_object("TT_inclusive"            ,75037895 ,831.76 ,ROOT.kRed-4)
...
#    MC_Sample["TT_TuneEE5C"           ]=mc_object("TT_TuneEE5C"             ,27034509 ,831.76 ,ROOT.kRed-4)
################# For FCNC ########################
    if Do_FCNC_Study:
        MC_Sample["FCNC_ST_tug"           ]=mc_object("FCNC_ST_tug"       ,1952509  ,FCNC_XS  ,ROOT.TColor.GetColor("#800000"))
        MC_Sample["FCNC_ST_tcg"           ]=mc_object("FCNC_ST_tcg"       ,1830326  ,FCNC_XS  ,ROOT.TColor.GetColor("#ffa500"))
################# For BSM ########################
...
################# For PDF uncertainty sample ####
    Sys_MC_Sample["DY_PDF_Up"                ]=mc_object("DY_PDF_Up"                       ,78605770 ,5765.4 ,ROOT.kRed-4)
    Sys_MC_Sample["DY_PDF_Down"              ]=mc_object("DY_PDF_Down"                     ,78605770 ,5765.4 ,ROOT.kRed-4)
################# For QCD uncertainty sample ####
    Sys_MC_Sample["DY_QCD_Up"                ]=mc_object("DY_QCD_Up"                       ,78605770 ,5765.4 ,ROOT.kRed-4)
    Sys_MC_Sample["DY_QCD_Down"              ]=mc_object("DY_QCD_Down"                     ,78605770 ,5765.4 ,ROOT.kRed-4)
#################################################
#    Sys_MC_Sample["TT_TuneCUETP8M2T4down"    ]=mc_object("TT_TuneCUETP8M2T4down"           ,27877257 ,831.76 ,ROOT.kRed-4)
    Sys_MC_Sample["TT_TuneCUETP8M2T4down"    ]=mc_object("TT_TuneCUETP8M2T4down"           ,57721717 ,831.76 ,ROOT.kRed-4)
#    Sys_MC_Sample["TT_TuneCUETP8M2T4up"      ]=mc_object("TT_TuneCUETP8M2T4up"             ,28701616 ,831.76 ,ROOT.kRed-4)
    Sys_MC_Sample["TT_TuneCUETP8M2T4up"      ]=mc_object("TT_TuneCUETP8M2T4up"             ,58144172 ,831.76 ,ROOT.kRed-4)
############### for color reconnection : envelope 
#    Sys_MC_Sample["TT_QCDbasedCRTune_erdON"  ]=mc_object("TT_QCDbasedCRTune_erdON"         ,28789634 ,831.76 ,ROOT.kRed-4)
    Sys_MC_Sample["TT_QCDbasedCRTune_erdON"  ]=mc_object("TT_QCDbasedCRTune_erdON"         ,57788977 ,831.76 ,ROOT.kRed-4)
    Sys_MC_Sample["TT_GluonMoveCRTune"       ]=mc_object("TT_GluonMoveCRTune"              ,56456001 ,831.76 ,ROOT.kRed-4)
    Sys_MC_Sample["TT_erdON"                 ]=mc_object("TT_erdON"                        ,58448827 ,831.76 ,ROOT.kRed-4)
#    Sys_MC_Sample["TT_erdON"                 ]=mc_object("TT_erdON"                        ,28550174 ,831.76 ,ROOT.kRed-4)
################# For PDF uncertainty sample ####
    Sys_MC_Sample["TT_PDF_Up"                ]=mc_object("TT_PDF_Up"                       ,77975776 ,87.31 ,ROOT.kRed-4)
    Sys_MC_Sample["TT_PDF_Down"              ]=mc_object("TT_PDF_Down"                     ,77975776 ,87.31 ,ROOT.kRed-4)
################# For QCD uncertainty sample ####
    Sys_MC_Sample["TT_QCD_Up"                ]=mc_object("TT_QCD_Up"                       ,77975776 ,87.31 ,ROOT.kRed-4)
    Sys_MC_Sample["TT_QCD_Down"              ]=mc_object("TT_QCD_Down"                     ,77975776 ,87.31 ,ROOT.kRed-4)
#################################################
#    Sys_MC_Sample["TT_fsrdown"               ]=mc_object("TT_fsrdown"                      ,29156223 ,831.76 ,ROOT.kRed-4)
    Sys_MC_Sample["TT_fsrdown"               ]=mc_object("TT_fsrdown"                      ,57563666 ,831.76 ,ROOT.kRed-4)
#    Sys_MC_Sample["TT_fsrup"                 ]=mc_object("TT_fsrup"                        ,29022611 ,831.76 ,ROOT.kRed-4)
    Sys_MC_Sample["TT_fsrup"                 ]=mc_object("TT_fsrup"                        ,58475264 ,831.76 ,ROOT.kRed-4)
#    Sys_MC_Sample["TT_fsrdown"               ]=mc_object("TT_fsrdown"                      ,14578111 ,831.76 ,ROOT.kRed-4)#just for FSR statistic test
#    Sys_MC_Sample["TT_fsrup"                 ]=mc_object("TT_fsrup"                        ,14511305 ,831.76 ,ROOT.kRed-4)#
...
#    Sys_MC_Sample["TT_hdampUP"               ]=mc_object("TT_hdampUP"                      ,29151802 ,831.76 ,ROOT.kRed-4)
    Sys_MC_Sample["TT_hdampDOWN"             ]=mc_object("TT_hdampDOWN"                    ,55809842 ,831.76 ,ROOT.kRed-4)
    Sys_MC_Sample["TT_hdampUP"               ]=mc_object("TT_hdampUP"                      ,58320199 ,831.76 ,ROOT.kRed-4)
#    Sys_MC_Sample["TT_isrdown"               ]=mc_object("TT_isrdown"                      ,28744718 ,831.76 ,ROOT.kRed-4)
    Sys_MC_Sample["TT_isrdown"               ]=mc_object("TT_isrdown"                      ,58421030 ,831.76 ,ROOT.kRed-4)
    Sys_MC_Sample["TT_isrup"                 ]=mc_object("TT_isrup"                        ,57577179 ,831.76 ,ROOT.kRed-4)
    Sys_MC_Sample["TT_mtop1695"              ]=mc_object("TT_mtop1695"                     ,58281931 ,831.76 ,ROOT.kRed-4)
#    Sys_MC_Sample["TT_mtop1695"              ]=mc_object("TT_mtop1695"                     ,19295090 ,831.76 ,ROOT.kRed-4)
    Sys_MC_Sample["TT_mtop1755"              ]=mc_object("TT_mtop1755"                     ,38909457 ,831.76 ,ROOT.kRed-4)
...
#    Sys_MC_Sample["ST_tW_fsrdown_top"        ]=mc_object("ST_tW_fsrdown_top"               ,1467797 ,19.47   ,ROOT.kRed-4)
#    Sys_MC_Sample["ST_tW_fsrup_top"          ]=mc_object("ST_tW_fsrup_top"                 ,1596163 ,19.47   ,ROOT.kRed-4)
    Sys_MC_Sample["ST_tW_herwigpp_top"       ]=mc_object("ST_tW_herwigpp_top"              ,3200997 ,19.47   ,ROOT.kRed-4)
    ...
    Sys_MC_Sample["ST_tW_fsrdown_antitop"    ]=mc_object("ST_tW_fsrdown_antitop"           ,3234964 ,19.47   ,ROOT.kRed-4)##just for test
    Sys_MC_Sample["ST_tW_fsrup_antitop"      ]=mc_object("ST_tW_fsrup_antitop"             ,3001527 ,19.47   ,ROOT.kRed-4)
#    Sys_MC_Sample["ST_tW_fsrdown_antitop"    ]=mc_object("ST_tW_fsrdown_antitop"           ,1617482 ,19.47   ,ROOT.kRed-4)
#    Sys_MC_Sample["ST_tW_fsrup_antitop"      ]=mc_object("ST_tW_fsrup_antitop"             ,1500763 ,19.47   ,ROOT.kRed-4)
...
################# For FCNC uncertainty sample ####
    Sys_MC_Sample["FCNC_tug_PS_Up"               ]=mc_object("FCNC_tug_PS_Up"              ,1977476 ,FCNC_XS ,ROOT.kRed-4)
    Sys_MC_Sample["FCNC_tug_PS_Down"             ]=mc_object("FCNC_tug_PS_Down"            ,1988568 ,FCNC_XS ,ROOT.kRed-4)
    Sys_MC_Sample["FCNC_tug_PDF_Up"              ]=mc_object("FCNC_tug_PDF_Up"             ,1972745 ,FCNC_XS ,ROOT.kRed-4)
    Sys_MC_Sample["FCNC_tug_PDF_Down"            ]=mc_object("FCNC_tug_PDF_Down"           ,1972745 ,FCNC_XS ,ROOT.kRed-4)
    Sys_MC_Sample["FCNC_tug_QCD_Up"              ]=mc_object("FCNC_tug_QCD_Up"             ,1972745 ,FCNC_XS ,ROOT.kRed-4)
    Sys_MC_Sample["FCNC_tug_QCD_Down"            ]=mc_object("FCNC_tug_QCD_Down"           ,1972745 ,FCNC_XS ,ROOT.kRed-4)
    Sys_MC_Sample["FCNC_tcg_PS_Up"               ]=mc_object("FCNC_tcg_PS_Up"              ,1978568 ,FCNC_XS ,ROOT.kRed-4)
    Sys_MC_Sample["FCNC_tcg_PS_Down"             ]=mc_object("FCNC_tcg_PS_Down"            ,1987421 ,FCNC_XS ,ROOT.kRed-4)
    Sys_MC_Sample["FCNC_tcg_PDF_Up"              ]=mc_object("FCNC_tcg_PDF_Up"             ,1850507 ,FCNC_XS ,ROOT.kRed-4)
    Sys_MC_Sample["FCNC_tcg_PDF_Down"            ]=mc_object("FCNC_tcg_PDF_Down"           ,1850507 ,FCNC_XS ,ROOT.kRed-4)
    Sys_MC_Sample["FCNC_tcg_QCD_Up"              ]=mc_object("FCNC_tcg_QCD_Up"             ,1850507 ,FCNC_XS ,ROOT.kRed-4)
    Sys_MC_Sample["FCNC_tcg_QCD_Down"            ]=mc_object("FCNC_tcg_QCD_Down"           ,1850507 ,FCNC_XS ,ROOT.kRed-4)
...
#        system_list.append("Sample_TT_erdON"                )
        ...
        system_list.append("Sample_DY_PDF_Up"            )
        system_list.append("Sample_DY_PDF_Down"          )
        system_list.append("Sample_DY_QCD_Up"            )
        system_list.append("Sample_DY_QCD_Down"          )
        system_list.append("Sample_TT_TuneEE5C"          )
        system_list.append("Sample_TT_CR_Up"             )
        system_list.append("Sample_TT_CR_Down"           )
        system_list.append("Sample_TT_PDF_Up"            )
        system_list.append("Sample_TT_PDF_Down"          )
        system_list.append("Sample_TT_QCD_Up"            )
        system_list.append("Sample_TT_QCD_Down"          )
...
#        system_list.append("Sample_ST_tW_fsrup"          )
        ...
        if Do_FCNC_Study:
            system_list.append("Sample_FCNC_PS_Up"   ) 
            system_list.append("Sample_FCNC_PS_Down" ) 
            system_list.append("Sample_FCNC_PDF_Up"  ) 
            system_list.append("Sample_FCNC_PDF_Down") 
            system_list.append("Sample_FCNC_QCD_Up"  ) 
            system_list.append("Sample_FCNC_QCD_Down") 
...
"bgk_TTTo2L2Nu_scale_up"                  :"TT\_up", 
"bgk_TTTo2L2Nu_scale_down"                :"TT\_down", 
...
"fsrup"                                   :"FSR\_up"                ,
"fsrdown"                                 :"FSR\_down"              ,
"Sample_DY_PDF_Up"                        :"DY\_PDF\_Up",
"Sample_DY_PDF_Down"                      :"DY\_PDF\_Down",
"Sample_DY_QCD_Up"                        :"DY\_QCD\_Up",
"Sample_DY_QCD_Down"                      :"DY\_QCD\_Down",
"Sample_TT_TuneCUETP8M2T4down"            :"TT\_Tune\_down",
"Sample_TT_TuneCUETP8M2T4up"              :"TT\_Tune\_up",
"Sample_TT_CR_Up"                         :"TT\_CR\_Up",
"Sample_TT_CR_Down"                       :"TT\_CR\_Down",
"Sample_TT_PDF_Up"                        :"TT\_PDF\_Up",
"Sample_TT_PDF_Down"                      :"TT\_PDF\_Down",
"Sample_TT_QCD_Up"                        :"TT\_QCD\_Up",
"Sample_TT_QCD_Down"                      :"TT\_QCD\_Down",
...
"fsrup"                       :"FSR_Up"                ,
"fsrdown"                     :"FSR_Down"              ,
"Sample_DY_PDF_Up"            :"DY_PDF_Up"             ,
"Sample_DY_PDF_Down"          :"DY_PDF_Down"           ,
"Sample_DY_QCD_Up"            :"DY_QCD_Up"             ,
"Sample_DY_QCD_Down"          :"DY_QCD_Down"           ,
"Sample_TT_TuneCUETP8M2T4down":"TT_Tune_Down"          ,
"Sample_TT_TuneCUETP8M2T4up"  :"TT_Tune_Up"            ,
"Sample_TT_CR_Up"             :"TT_CR_Up"              ,
"Sample_TT_CR_Down"           :"TT_CR_Down"            ,
"Sample_TT_PDF_Up"            :"TT_PDF_Up"             ,
"Sample_TT_PDF_Down"          :"TT_PDF_Down"           ,
"Sample_TT_QCD_Up"            :"TT_QCD_Up"             ,
"Sample_TT_QCD_Down"          :"TT_QCD_Down"           ,
...
"Sample_ST_tW_mtop1755"       :"tw_TopMass_Up"         }
    dict_FCNC_system_name_limit={
"Sample_FCNC_PS_Up"           :"FCNC_PS_Up"            ,
"Sample_FCNC_PS_Down"         :"FCNC_PS_Down"          ,
"Sample_FCNC_PDF_Up"          :"FCNC_PDF_Up"           ,
"Sample_FCNC_PDF_Down"        :"FCNC_PDF_Down"         ,
"Sample_FCNC_QCD_Up"          :"FCNC_QCD_Up"           ,
"Sample_FCNC_QCD_Down"        :"FCNC_QCD_Down"         }
    dir_system_name={}
    ...
    dir_MVA={"H_MLP_0jet_0bjet":"MLP(0jet,0bjet)","H_MLP_1jet_0bjet_comb":"MLP(1jet,0bjet)","H_MLP_1jet_1bjet_comb":"MLP(1jet,1bjet)","H_MLP_2jet_1bjet_comb":"MLP(2jet,1bjet)","H_MLP_2jet_2bjet":""}#EFTfinal 
    #dir_MVA={"H_0jet_0bjet":"(0jet,0bjet)","H_1jet_0bjet":"(1jet,0bjet)","H_1jet_1bjet":"(1jet,1bjet)","H_2jet_1bjet":"(2jet,1bjet)","H_2jet_2bjet":"(2jet,2bjet)"}###for Cg 1bin final
    #dir_MVA={"H_MLP_tug_xjet_1bjet":"MLP(xjet,1bjet)"}#FCNC final
    #dir_MVA={"H_MLP_tcg_xjet_1bjet":"MLP(xjet,1bjet)"}#FCNC final
    #dir_MVA={"H_BSM_0j0b_Ptll":"","H_BSM_1j0b_Ptll":"","H_BSM_1j1b_Ptll":"","H_BSM_2j1b_Ptll":"","H_MLP_2jet_2bjet":""}#check EFT  using ptll 
    #dir_MVA={"H_BSM_xj1b_Ptll":"tug"}#check FCNC  using ptll 
    #dir_MVA={"H_BSM_xj1b_Ptll":"tcg"}#check FCNC  using ptll 
...
"H_BSM_xj1b_l1_pt"         : "P_{T}^{leading lepton} (GeV/c)",
"H_BSM_xj1b_Ptll"          : "P_{T}^{ll} (GeV/c)",
"H_BSM_xj1b_dPt_l1_l2"     : "#DeltaP_{T}(l,l)",
"H_BSM_xj1b_Mll"           : "M_{ll} (GeV/c^{2})",
"H_BSM_xj1b_Cll"           : "Centrality(ll)",
"H_BSM_xj1b_ll_j1_pt"      : "P_{T}^{ll,jet^{leading}} (GeV/c)",
"H_BSM_xj1b_dR_ll_j1"      : "#DeltaR(ll,jet^{leading})",
"H_BSM_xj1b_M_l2_j1"       : "M(lepton^{sub-leading},jet^{leading}) (GeV/c^{2})",
"H_BSM_xj1b_Cl1_j1"        : "Centrality(lljet^{leading})",
...
#    dir_hist.update(dir_MVA_all)
    dir_hist.update(dir_BSM_MVA_variables_v1)##for limit check using ptll
    if Do_FCNC_Study:
    ...
    if do_same_sign:
        ...
        for hname in all_system_hist_ss:
            for nsys in all_system_hist_ss[hname]:
                ...
                ############ print SS #####################################
                if Print_SS and hname=="H_Limit_N_jet_bjet" and nsys=="nominal":
                    h_ss_data =h_ss.Clone("%s_%s_ss_data"%(hname,nsys))
                    h_ss_TW   =h_ss.Clone("%s_%s_ss_TW"%(hname,nsys))
                    h_ss_TT   =h_ss.Clone("%s_%s_ss_TT"%(hname,nsys))
                    h_ss_DY   =h_ss.Clone("%s_%s_ss_DY"%(hname,nsys))
                    h_ss_other=h_ss.Clone("%s_%s_ss_other"%(hname,nsys))
                    h_ss_TW   .Scale(0)
                    h_ss_TT   .Scale(0)
                    h_ss_DY   .Scale(0)
                    h_ss_other.Scale(0)
                    for sname in all_system_hist_ss[hname][nsys]:
                        if "TW_"         in sname:h_ss_TW.Add(all_system_hist_ss[hname][nsys][sname])
                        elif "TTTo2L2Nu" in sname:h_ss_TT.Add(all_system_hist_ss[hname][nsys][sname])
                        elif "DY"        in sname:h_ss_DY.Add(all_system_hist_ss[hname][nsys][sname])
                        elif ("data" not in sname and "WJet_mad" not in sname and "FCNC" not in sname):h_ss_other.Add(all_system_hist_ss[hname][nsys][sname])
                    for tmp_hist in [h_ss_data,h_ss_TW,h_ss_TT,h_ss_DY,h_ss_other]:
                        print "SS for %s|"%(tmp_hist.GetName()),
                        for ij in range(1,tmp_hist.GetNbinsX()+1):
                            print "%.0f |"%(tmp_hist.GetBinContent(ij)),
                        print "\n"
                ###################################################################
                for sname in all_system_hist_ss[hname][nsys]:
        ...
        if True:## make Jets from nominal noly
            for hname in all_system_hist:
                for nsys in all_system_hist[hname]:
                    if "luminosity" in nsys or "bgk_WJet" in nsys:continue
                    all_system_hist[hname][nsys]["WJet_mad"]=all_system_hist[hname]["nominal"]["WJet_mad"]
    ...
    for hname in hist_sys_mc_all:
        ...
        for nsys in hist_sys_mc_all[hname]:
            ...
            if N_sys>0:
                ...
                if "bgk" in nsys and "bgk_DYJets" not in nsys and "bgk_WJet" not in nsys and "bgk_TTTo2L2Nu" not in nsys:
...
#        Draw_all_system_ratio_v2(dir, date, hist_sys_mc_TT , Files, dir_hist, dir_system_name, system_list, ["Sample_TT","isr","fsr"],"TT_Samples")
#        Draw_all_system_ratio_v2(dir, date, hist_sys_mc_TW , Files, dir_hist, dir_system_name, system_list, ["Sample_ST_tW","isr","fsr"],"TW_Samples")
#        Draw_all_system_ratio_v2(dir, date, hist_sys_mc_TT , Files, dir_hist, dir_system_name, system_list, ["fsr"],"TT_Samples")
#        Draw_all_system_ratio_v2(dir, date, hist_sys_mc_TW , Files, dir_hist, dir_system_name, system_list, ["fsr"],"TW_Samples")
#        Draw_all_system_ratio_v2(dir, date, hist_sys_mc_TT , Files, dir_hist, dir_system_name, system_list, ["Sample_TT_QCD"],"TT_Samples")
    ...
    BSM_Hist={}
    if do_save_sys_hist:
        no_signal=False # true is for tw measurement
        Ctg_LO=False
        system_uncert_list=system_list
        dir_sf={str(1.00):"1.00", str(0.05):"0.05", str(0.10):"0.10", str(0.15):"0.15", str(0.20):"0.20", str(0.25):"0.25", str(0.30):"0.30", str(0.02):"0.02",str(0.50):"0.50", str(1.50):"1.50", str(2.00):"2.00", str(2.50):"2.50", str(3.00):"3.00", str(4.50):"4.50", str(5.00):"5.00",str(7.00):"7.00",str(8.50):"8.50"}
        for sf in [1.00, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 7.00, 0.50, 4.50, 2.00 ]:
            BSM_Hist[dir_sf[str(sf)]]={}
            Frist=True 
            for do_opposite in [True,False]:
                opp="opp" if do_opposite else "nom"
                BSM_Hist[dir_sf[str(sf)]][opp]={}
                #for choose_C in ["Ctg","Ctw","Cphiq","Cg","Cphig"]:
                for choose_C in ["Ctg","Ctw","Cphiq","Cg"]:
                    BSM_Hist[dir_sf[str(sf)]][opp][str(choose_C)]={}
                    ...
                    elif choose_C == "Cg":for_Cg=True
                    elif choose_C == "Cphig":for_Cphig=True
                    else: print "wrong"
                    if no_signal:
                        for_Ctg  =False
                        for_Cphiq=False
                    ...
                    signal_tw_Cphiq_xs =float((55.24+6.7*1.32*sf+0.21*1.31*sf*sf)*(1-0.676*0.676)/2)
                    signal_tw_Ctw_xs   =float((55.24-4.45*1.27*sf+1*1.18*sf*sf)*(1-0.676*0.676)/2)
                    signal_tw_Ctg_xs   =float((55.24+6.55*1.27*0.5*sf+4.99*1.06*0.25*sf*sf)*(1-0.676*0.676)/2)
                    signal_ttbar_Ctg_xs=float(76.63+405.66*0.324*0.324*0.5*sf + 94.176*0.324*0.324*0.25*sf*sf )
                    signal_ttbar_Cg_xs =float(76.63+25.33*0.324*0.324*sf + 80.3*0.324*0.324*sf*sf )
                    signal_ttbar_Cphig_xs =float(76.63+25.33*0.324*0.324*sf + 80.3*0.324*0.324*sf*sf )
                    if Ctg_LO:
                    ...
                    if do_opposite:
                        ...
                        if Ctg_LO:
                            signal_ttbar_Ctg_xs=float(76.63-274.1*0.324*0.324*sf + 65.4*0.324*0.324*sf*sf )
                    signalonly_tw_Cphiq_xs  =float((0.21*1.31*sf*sf)*(1-0.676*0.676)/2)
                    signalonly_tw_Ctw_xs   =float((1*1.18*sf*sf)*(1-0.676*0.676)/2)
                    signalonly_tw_Ctg_xs   =float(4.99*1.06*0.25*sf*sf*(1-0.676*0.676)/2)
                    signalonly_ttbar_Ctg_xs=float(94.176*0.324*0.324*0.25*sf*sf )
                    signalonly_ttbar_Cg_xs =float(80.3*0.324*0.324*sf*sf )
                    signalonly_ttbar_Cphig_xs =float(80.3*0.324*0.324*sf*sf )
                    if Ctg_LO:
                    ...
                    for region in dir_MVA:
                        ...
                        elif "xjet_1bjet" in region or "xj1b" in region :str_extra="_xjet_1bjet"
                      ...
                      #  for nsys in all_system_hist[region]:
                        for nsys in system_uncert_list:
                            ...
                            for sname in all_system_hist[region][nsys]:
                                if "data" in sname:h_data.Add(all_system_hist[region][nsys][sname],1)
                                    ...
                                    if for_Cphiq or for_Ctw or for_Ctg:
                                        #h_tw.Add(all_system_hist[region][nsys][sname],float(55.24/71.74))## scale  to LO
                                        h_tw.Add(all_system_hist[region][nsys][sname],1)## NLO
                                    ...
                                    if for_Ctg or for_Cg or for_Cphig:
                                        ...
                                        h_ttbar     .Add(all_system_hist[region][nsys][sname],1)##for ttbar NNLO
                                    else:
                                        h_ttbar.Add(all_system_hist[region][nsys][sname],1)
                                    if for_Ctg :
                                        ...
                                        h_ttbar_signal    .Add(all_system_hist[region][nsys][sname],float(signal_ttbar_Cg_xs/MC_Sample["TTTo2L2Nu"].crosssection))
                                        h_ttbar_signalonly.Add(all_system_hist[region][nsys][sname],float(signalonly_ttbar_Cg_xs/MC_Sample["TTTo2L2Nu"].crosssection))
                                    elif for_Cphig :
                                        h_ttbar_signal    .Add(all_system_hist[region][nsys][sname],float(signal_ttbar_Cphig_xs/MC_Sample["TTTo2L2Nu"].crosssection))
                                        h_ttbar_signalonly.Add(all_system_hist[region][nsys][sname],float(signalonly_ttbar_Cphig_xs/MC_Sample["TTTo2L2Nu"].crosssection))
                            ...
                            ############## Save hist for EFT plot ###
                            if nsys=="nominal":
                                h_BSM                 = h_tw_signal       .Clone("BSM_"   +dir_sf[str(sf)]+"_"+opp+"_"+str(choose_C)+"_"+region+"_"+sys_name)
                                if choose_C =="Ctg":
                                    h_BSM.Add(h_ttbar_signal)
                                elif choose_C =="Ctw" or choose_C =="Cphiq":
                                    h_BSM.Add(h_ttbar)
                                elif choose_C =="Cg" or choose_C =="Cphig":
                                    h_BSM.Add(h_ttbar_signal)
                                    h_BSM.Add(h_tw)
                                h_BSM.Add(h_tw_NLO)
                                h_BSM.Add(h_ttbar_NNLO)
                                h_BSM.Add(h_DY)
                                h_BSM.Add(h_jets)
                                h_BSM.Add(h_other)
                                BSM_Hist[dir_sf[str(sf)]][opp][str(choose_C)][region]=h_BSM
                            #########change zero bin ########
                            ...
                            if nsys=="nominal":
                                ...
                                if no_signal==False and Do_FCNC_Study==False:
                                    ...
                                    if "tug" in region:
                                        ...
                                        #print "Warning: no tug or tcg !!!"
                                        tmp_hist=all_system_hist[region][nsys]["FCNC_ST_%s"%dir_MVA[region]].Clone(str_fcnc)
                                        tmp_hist.Scale(float(sf))
                                        tmp_hist.Write()
                                h_tw_out      .Write()
                                h_ttbar_out   .Write()
                            ...
                            #    h_other_out   .Write(h_other_out   .GetName()+"_TT_CR_Down")
                                ...
                                if no_signal==False and Do_FCNC_Study==False:
                                    ...
                                    if "tug" in region:
                                        ...
                                        #print "Warning: no tug or tcg!!!"
                                        tmp_hist=all_system_hist[region][nsys]["FCNC_ST_%s"%dir_MVA[region]].Clone(str_fcnc+sys_name)
                                        tmp_hist.Scale(float(sf))
                                        tmp_hist.Write()
                                h_tw_out      .Write()
                                h_ttbar_out   .Write()
...
#    sample_list=["WJet_mad","GGHWWTo2L2Nu","VBFHWWTo2L2Nu","WG_LNuG","ZZ_4L","ZZ_2L2Nu","WZ_2L2Q","WZ_3LNu","WWTo2L2Nu","TTGJets","TTZToQQ","TTZToLLNuNu","TTWJetsToLNu","TTWJetsToQQ","DYJetsToLL_M50_amc","DYJetsToLL_M10to50_amc","TT_TuneEE5C","TW_antitop","TW_top"]
    if Do_FCNC_Study:
    ...
    if do_plot_main:
        ...
        for hist in dir_hist:
            if all_system_hist[hist]["nominal"]["data"]:
                ...
                h_tW.Scale(0)
                h_DY    =h_data.Clone("h_DY_%s"%(hist))
                h_DY.Scale(0)
                h_Other =h_data.Clone("h_Other_%s"%(hist))
                h_Other.Scale(0)
                ...
                for sname in sample_list:
                    ...
                    if hist == "H_Limit_N_jet_bjet":
                        print "%s|1j0t| %s|%.1f +- %.1f| %.1f%%|"%(Channel,sname, h_tmp.GetBinContent(2),h_tmp.GetBinError(2),100*h_tmp.GetBinContent(2)/49960.6)
                        print "%s|1j1t| %s|%.1f +- %.1f| %.1f%%|"%(Channel,sname, h_tmp.GetBinContent(3),h_tmp.GetBinError(3),100*h_tmp.GetBinContent(3)/37673.0)
                    h_tmp.SetStats(0)
                    ...
                    if sname == "TTTo2L2Nu":
                        ...
                        h_tW.Add(h_tmp)
                    elif sname in ["DYJetsToLL_M50_amc","DYJetsToLL_M10to50_amc"]:
                        h_DY.Add(h_tmp)
                    elif sname in ["WJet_mad","GGHWWTo2L2Nu","VBFHWWTo2L2Nu","WG_LNuG","ZZ_4L","ZZ_2L2Nu","WZ_2L2Q","WZ_3LNu","WWTo2L2Nu","TTGJets","TTZToQQ","TTZToLLNuNu","TTWJetsToLNu","TTWJetsToQQ"]:
                        h_Other.Add(h_tmp)
                ...
                if draw_EFT_plot and hist in dir_MVA: 
                    for xsf in BSM_Hist:
                        #if xsf not in ["0.20"]:continue
                        if xsf not in ["0.50"]:continue
                        for ysf in BSM_Hist:
                           #if ysf not in ["2.00"]:continue
                           if ysf not in ["7.00"]:continue
                           for zsf in BSM_Hist:
                               if zsf not in ["4.50"]:continue
                               EFT_dict={}
                               EFT_dict["Ctg_"+str(xsf)]=BSM_Hist[xsf]["nom"]["Ctg"][hist]
                               EFT_dict["Ctw_"+str(ysf)]=BSM_Hist[ysf]["nom"]["Ctw"][hist]
                               EFT_dict["Cphiq_"+str(zsf)]=BSM_Hist[zsf]["nom"]["Cphiq"][hist]
                               draw_plots(Channel, date, dir, stack, h_mc, h_data, gr_stat_sys, hist, dir_hist[hist], ratio_style,log_hist_name, h_FCNC_tug, h_FCNC_tcg,EFT_dict)
                ...
                if do_print_sys_region:
                    if hist == "H_Limit_N_jet_bjet":
                        ...
                        for ibin in range(1,h_data.GetNbinsX()+1):
                            ...
                            N_DY     =h_DY.GetBinContent(ibin)
                            err_DY   =h_DY.GetBinError(ibin)
                            N_Other  =h_Other.GetBinContent(ibin)
                            err_Other=h_Other.GetBinError(ibin)
                            #print "%d bin & %s:%d$\pm$%d(stat.) & %s:%.0f$\pm$%.0f(stat.) & %s:%.0f$\pm$%.0f(stat.) & %s:%.0f$\pm$%.0f(stat.+syst.) & %s:%.3f$\pm$%.3f(stat.+syst.) "%(ibin,"data",N_data,err_data,"tW",N_tW,err_tW,"ttbar",N_ttbar,err_ttbar,"mc",N_mc,err_mc_stat_sys,"data/mc",ratio,err_ratio)
                            print "%d bin & %s:%d$\pm$%d(stat.) & %s:%.3f$\pm$%.3f(stat.) & %s:%.3f$\pm$%.3f(stat.)& %s:%.3f$\pm$%.3f(stat.) & %s:%.3f$\pm$%.3f(stat.) & %s:%.3f$\pm$%.3f(stat.+syst.) & %s:%.3f$\pm$%.3f(stat.+syst.) "%(ibin,"data",N_data,err_data,"tW",N_tW,err_tW,"ttbar",N_ttbar,err_ttbar,"DY",N_DY,err_DY,"Other",N_Other,err_Other,"mc",N_mc,err_mc_stat_sys,"data/mc",ratio,err_ratio)
