...
#If reverse, will fill events not in list, if not reverse, will fill events in list
	...
	flag_store = False
        ...
        if tree_in.n_jet == 1 and tree_in.n_bjet == 0 and tree_in.leading_pt < 3000 and tree_in.sub_leading_pt < 3000:
            n_fired += 1
            if not reverse:
                flag_store = True
        else:
...
if options.sample_name == "null":
	for sample in input_dic:
		event_filter(input_dic[sample], os.path.join("MC_data_80",input_dic[sample]["input_file"]), os.path.join(cut,input_dic[sample]["input_file"][:-5]+"_1j0b_EMu"+input_dic[sample]["input_file"][-5:]), cut)
else:
	isSplit_mode = True
	sample_list = options.sample_name.replace(" ","").split(",")
	for sample in sample_list:
		event_filter(input_dic[sample], os.path.join("MC_data_80",input_dic[sample]["input_file"]), os.path.join(cut,"split/"+input_dic[sample]["input_file"][:-5]+"_1j0b_EMu"+input_dic[sample]["input_file"][-5:]), cut)
