...
parser.add_option("-k", action="store_true", dest="do_k_scan",default=False)
...
#    Cats["obs"]["Diff_8TeV_7TeV_Inclusive"]      =cat_object( "obs_Diff_8TeV_7TeV_Inclusive"      , 87.364266 , 1 , 69, NDOF-NPara-3    , 0.85,1.05,201,0 ,0.4 ,201,0 ,0.2 ,201)##for final result, no D0,CDF R, fix R to CMS
    ...
    if do_k_scan :
         ##For Vtb=1, Vts=Vtd=0
        Cats["exp"]["Diff_8TeV_Ratio"]            =cat_object( "exp_Diff_8TeV_Ratio"               ,0 , 1 , 51, 52-NPara        , 0.85 ,1.05 ,401,0.0 ,0.15 ,401,0 ,0.15 ,401)
        Cats["obs"]["Diff_8TeV_Ratio"]           =cat_object( "obs_Diff_8TeV_Ratio"               , 52.996654 , 1 , 51, 52-NPara       , 0.85 ,1.05 ,401,0.0 ,0.15 ,401,0 ,0.15 ,401)
        ##For SM
        Cats["exp"]["Diff_8TeV_Ratio"]            =cat_object( "exp_Diff_8TeV_Ratio"               ,0.004797 , 1 , 51, 52-NPara        , 0.85 ,1.05 ,401,0.0 ,0.15 ,401,0 ,0.15 ,401)
        Cats["obs"]["Diff_8TeV_Ratio"]           =cat_object( "obs_Diff_8TeV_Ratio"               , 53.374155, 1 , 51, 52-NPara       , 0.85 ,1.05 ,401,0.0 ,0.15 ,401,0 ,0.15 ,401)
    ...
    k_array   =np.linspace(0.8,1.2,10001) 
    k_array1  =np.linspace(0.8,1.2,10001) 
    ...
    if len(np.where(k_array==1)[0])==0:k_array1=np.insert(k_array,0,values=[[1.0]],axis=0)
    #Fixed_Vtd = 0
    #Fixed_Vts = 0
    #Fixed_Vtb = 1
    Fixed_Vtd = 0.00869
    ...
    for ik in k_array1: 
        ...
        if marginlized:
            ...
            if str_method == "dchi2":
                K_value.append(ik)
                K_dchi2.append(tmp_chi2-Min_chi2)
                if (tmp_chi2-Min_chi2)<delta_chi2_cut_68:
                    K_band_1sigma.append(ik)
                    K_band_2sigma.append(ik)
                    K_band_3sigma.append(ik)
                ...
                print "wrong!!!"
                exit()
    print "min_chi2: %f, best fit k: %f"%(min_chi2, best_fit[0])
    print "68%% band: %f to %f"%(min(K_band_1sigma), max(K_band_1sigma))
    print "95%% band: %f to %f"%(min(K_band_2sigma), max(K_band_2sigma))
    gr_k_scan=ROOT.TGraph(len(K_value),np.array(K_value),np.array(K_dchi2))
    gr_k_scan.SetName("%s_%s_%f_%f_%f"%(str_type,icat,Fixed_Vtb,Fixed_Vts,Fixed_Vtd))
    F_out=ROOT.TFile("/user/wenxing/ST_TW_channel/CMSSW_8_0_25/src/Phynomenological_study/Single_TOP/limit_K_%s_%s.root"%(str_method,icat),"UPDATE")
    F_out.cd()
    gr_k_scan.Write("",ROOT.TObject.kOverwrite)
    F_out.Close()
#    exit()###stop here
    ...
    x_min = min(K_band_2sigma)-0.05
    x_max = min(K_band_2sigma)+0.05
    dummy = ROOT.TH2D("dummy","",1,x_min,x_max,1,0,6)
    dummy.SetStats(ROOT.kFALSE)
    dummy.GetYaxis().SetTitle("#Delta#Chi^{2}")
...
#    dummy.GetYaxis().CenterTitle()
    ...
    line_1sigma.Draw()
    line_2sigma.Draw()
    canvas.Print('/user/wenxing/ST_TW_channel/CMSSW_8_0_25/src/Phynomenological_study/Single_TOP/limit_K_%s_%s%s_%s_%f_%f_%f.png'%(str_method,icat,str_lumi,str_type,Fixed_Vtb,Fixed_Vts,Fixed_Vtd))
