...
#    Cats["obs"]["Diff_8TeV_7TeV_Inclusive"]      =cat_object( "obs_Diff_8TeV_7TeV_Inclusive"      , 87.364266 , 1 , 69, NDOF-NPara-3    , 0.85,1.05,201,0 ,0.4 ,201,0 ,0.2 ,201)##for final result, no D0,CDF R, fix R to CMS
    ...
    for vtb in vtb_array1: 
        for vtd in vtd_array1: 
            if do_Fix_R : 
                tmp0 = ((1-Fixed_R)*vtb*vtb/Fixed_R) - (vtd*vtd)
                if tmp0<0:continue
                vts_array1 = [math.sqrt(tmp0)]
...
#    print VtsVtd_2sigma
    ...
    if indiv_Vtb or indiv_Vts or indiv_Vtd or marginlized:
        F_out=ROOT.TFile("/user/wenxing/ST_TW_channel/CMSSW_8_0_25/src/Phynomenological_study/Single_TOP/limit_out/FixR2_limit_1D_%s_%s%s_%s%s.root"%(str_method,icat,str_lumi,str_type,str_fix_R),"UPDATE")
        F_out.cd()
...
################ save it for comparation ############################################
    ...
    f_out=ROOT.TFile("/user/wenxing/ST_TW_channel/CMSSW_8_0_25/src/Phynomenological_study/Single_TOP/limit_out/FixR2_limit_2D_%s_%s%s_%s%s.root"%(str_method,icat,str_lumi,str_type,str_fix_R),"UPDATE")
    f_out.cd()
