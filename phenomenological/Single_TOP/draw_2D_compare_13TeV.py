...
####    ############ Draw ############################################
def draw_compare(igr,gr_L, gr_S, gr_8):
    ...
    gr_L[1].SetMarkerColor(ROOT.TColor.GetColor("#00ffff"))
    gr_L[1].SetFillColor  (ROOT.TColor.GetColor("#00ffff"))
    gr_L[1].SetLineColor  (ROOT.TColor.GetColor("#00ffff"))
    ...
    gr_8[0].SetMarkerColor(ROOT.TColor.GetColor("#ff00ff"))
    gr_8[0].SetFillColor  (ROOT.TColor.GetColor("#ff00ff"))
    gr_8[0].SetLineColor  (ROOT.TColor.GetColor("#ff00ff"))
    gr_8[0].SetMarkerStyle(26)
    gr_8[0].SetMarkerSize(1.5)
    gr_8[0].SetLineWidth  (width_line)
    gr_8[1].SetMarkerColor(ROOT.TColor.GetColor("#0000ff"))
    gr_8[1].SetFillColor  (ROOT.TColor.GetColor("#0000ff"))
    gr_8[1].SetLineColor  (ROOT.TColor.GetColor("#0000ff"))
    gr_8[1].SetMarkerStyle(8)
    gr_8[1].SetLineWidth  (width_line)
    gr_8[2].SetMarkerColor(ROOT.TColor.GetColor("#76eec6"))
    gr_8[2].SetFillColor  (ROOT.TColor.GetColor("#76eec6"))
    gr_8[2].SetLineColor  (ROOT.TColor.GetColor("#76eec6"))
    gr_8[2].SetMarkerStyle(8)
    gr_8[2].SetLineWidth  (width_line)
    gr_8[3].SetMarkerColor(ROOT.TColor.GetColor("#6cbaf9"))
    gr_8[3].SetFillColor  (ROOT.TColor.GetColor("#6cbaf9"))
    gr_8[3].SetLineColor  (ROOT.TColor.GetColor("#6cbaf9"))
    gr_8[3].SetMarkerStyle(8)
    gr_8[3].SetLineWidth  (width_line)
    if only_draw_L:
...
#        gr_L[0].Draw("p")
    dummy.Draw("AXISSAME")
    legend = ROOT.TLegend(0.17,0.55,0.85,0.88)
    if only_draw_L:
...
#        legend.AddEntry(gr_S[0],'best-fit %s'%str_S,'p')
#        legend.AddEntry(gr_L[0],'best-fit %s'%str_L,'p')
        legend.AddEntry(gr_8[1],'68%% CL %s'%str_8     ,'f')
        legend.AddEntry(gr_L[1],'68%% CL %s'%str_L     ,'f')
        legend.AddEntry(gr_S[1],'68%% CL %s'%str_S     ,'f')
#        legend.AddEntry(gr_S[2],'95%% CL %s'%str_S     ,'f')
...
draw_S="F"
Dir="./limit_out/"
#chan="#sigma_{diff}^{t-ch} (LHC 8TeV)"
#chan="LHC + Tevatron"
chan="#sigma_{diff}^{t-ch} (LHC)"
...
str_S='Expected, 13TeV, 3000 fb^{-1}'
F_L=ROOT.TFile(Dir+"limit_2D_dchi2_Diff_13TeV_Ratio300fb_exp.root","read")
str_L='Expected, 13TeV, 300 fb^{-1}'
F_8=ROOT.TFile(Dir+"limit_2D_dchi2_Diff_8TeV_Ratio_exp.root","read")
str_8='Expected, 8TeV, 20.2 fb^{-1}'
...
gr_dicti["VtbVts"]["8"]=[F_8.Get("VtbVts_best"),F_8.Get("VtbVts_1s"),F_8.Get("VtbVts_2s"),F_8.Get("VtbVts_3s")]
gr_dicti["VtbVtd"]["8"]=[F_8.Get("VtbVtd_best"),F_8.Get("VtbVtd_1s"),F_8.Get("VtbVtd_2s"),F_8.Get("VtbVtd_3s")]
gr_dicti["VtsVtd"]["8"]=[F_8.Get("VtsVtd_best"),F_8.Get("VtsVtd_1s"),F_8.Get("VtsVtd_2s"),F_8.Get("VtsVtd_3s")]
for igr in gr_dicti:
    gr_L=gr_dicti[igr]["L"]
    gr_S=gr_dicti[igr]["S"]
    gr_8=gr_dicti[igr]["8"]
    draw_compare(igr,gr_L, gr_S, gr_8)
print "done!"
